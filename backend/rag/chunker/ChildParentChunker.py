    ...
    def __init__(
        ...
        if self.unit == "token":
            self.token_counter = (
                ...
                else get_default_token_counter()
            )
        else:
    ...
    def chunk_units_for_doc(self, doc_id: str, units: List[Dict[str, Any]]) -> ParentChildChunkResult:
        ...
        for parent_index, parent_units in enumerate(parent_unit_groups, start=1):
            ...
            parent = build_parent_chunk_v1(
                parent_chunk_id=parent_chunk_id,
                doc_id=doc_id,
                tenant_id=self._first_non_empty(parent_units, "tenant_id", "default"),
                kb_id=self._first_non_empty(parent_units, "kb_id", "default"),
                file_id=self._first_non_empty(parent_units, "file_id"),
        ...
        *,
        ...
        for child_index_in_parent, (child_text, start, end) in enumerate(windows, start=1):
            ...
            child = build_child_chunk_v1(
                ...
                parent_chunk_id=parent_chunk_id,
                doc_id=doc_id,
                tenant_id=self._first_non_empty(child_units, "tenant_id", "default"),
                kb_id=self._first_non_empty(child_units, "kb_id", "default"),
                file_id=self._first_non_empty(child_units, "file_id"),
                source_type=self._first_non_empty(parent_units, "source_type", DEFAULT_SOURCE_TYPE),
                text=child_text,
