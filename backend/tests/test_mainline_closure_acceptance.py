...
def test_corrective_retrieval_branch_runs_and_recovers(tmp_path) -> None:
    ...
    tool_started = [item for item in events if item.get("event_type") == "tool_started"]
    assert len(tool_started) == 3
    assert all(item.get("call_id") for item in tool_started)
    assert any("recovery_1" in item["call_id"] for item in tool_started)
...
def test_step_16_audit_identifies_noop_gates_and_coarse_scheme_writer() -> None:
    ...
    assert by_id["generation_checker_noop"]["present"] is True
    assert by_id["repair_strategy_noop"]["present"] is True
    assert by_id["evidence_grader_noop"]["present"] is True
    assert by_id["section_aware_retrieval_present"]["present"] is True
    assert report["summary"]["active_risk_count"] >= 3
